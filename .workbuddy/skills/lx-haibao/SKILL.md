---
name: lx-haibao
description: 企业版 WorkBuddy 专用司机活动海报生成。用于从城市活动 TXT 生成哈啰、安安用车、蔚蓝滴仕、江南出行等已配置品牌海报；按 check -> dry-run 确认 -> confirmed sample/full generation 三阶段执行，自动按 TXT 文件名识别品牌，通过图片 Provider Adapter 优先调用 AIHubMix，失败后按规则回退 APIMart / 火山方舟 Seedream，并逐张验证二维码。不要使用 Codex image_gen。
---
...
本 skill 是 WorkBuddy 独立包，不依赖原 Codex 项目的 `image_gen`。正式生图通过图片 Provider Adapter 完成，默认优先使用 AIHubMix；连接超时、网络错误、5xx、429 等供应商可用性问题再按配置顺序回退 APIMart / 火山方舟 Seedream。
...
- `providers`：默认 `["aihubmix", "apimart", "volcengine_seedream"]`，按顺序尝试。
- `provider_primary`：默认 `aihubmix`，兼容旧配置。
- `provider_fallback`：默认 `apimart`。
- `aihubmix.api_key`：AIHubMix API Key，也可用 `AIHUBMIX_API_KEY` 环境变量。
- `aihubmix.base_url`：默认 `https://aihubmix.com/v1`。
- `aihubmix.task_base_url`：默认 `https://api.aihubmix.com/v1`，用于异步任务轮询。
- `aihubmix.model`：默认 `gpt-image-2`。
- `aihubmix.endpoint`：默认 `images_edits`，用于带品牌 logo、二维码和模板参考图生成；如需切换统一模型接口，可设为 `predictions`。
- `aihubmix.quality`：默认 `low`，用于降低 AIHubMix edits 长请求超时概率。
- `aihubmix.output_format`：默认 `png`。
- `aihubmix.stream`：默认 `false`；当前 AIHubMix edits 流式响应可能只返回 request_id 但没有图片事件。
- `aihubmix.partial_images`：默认 `2`，控制流式返回的中间图数量。
...
- `AIHUBMIX_IMAGE_ENDPOINT`
- `AIHUBMIX_IMAGE_QUALITY`
- `AIHUBMIX_OUTPUT_FORMAT`
- `AIHUBMIX_IMAGE_STREAM`
...
`--check-providers` 只检查配置和 `base_url` 连通性，不调用图片生成接口，不产生生图费用。正式生图时，400、401、403 等参数或鉴权错误不会自动 fallback；连接超时、网络错误、5xx、429、供应商不可用才会尝试下一个 provider。若某个 provider 成功返回图片但二维码验证失败，下一次 QR 重试会跳过该 provider，改用后备 provider。

如需确认某个 provider 的 key、额度、模型和图片端点是否真的可出图，运行最小真实生图检查：

```bash
<.venv Python> scripts/run_poster_batch.py --smoke-provider aihubmix --confirmed
```

`--smoke-provider` 会真实调用图片生成接口，默认生成一张 `1024x1024`、`low` 质量测试图，可能产生费用；它不读取 TXT、不使用模板/logo/二维码，也不做二维码验证。
