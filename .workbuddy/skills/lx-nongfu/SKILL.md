...
1. 大文档来源：本地 Excel 路径，或腾讯文档在线表格 URL。
...
- 腾讯文档在线表格：先从 URL 提取短 ID，调用 `query_file_info` 获取真实 `file_id`，再 `sheet.get_info` / `sheet.get_range`。
- 普通 Word/PDF/腾讯文档正文：先说明需要转成结构化表格或明确表格区域，再继续。
