...
| `lx-txdocs` | 批量发布 Excel 到个人版腾讯文档（Open API V3） | 发布 | `lxx_share`, `lx_shujuku` |
| `lx-txsaasdocs` | 腾讯文档企业版/SaaS API 接入，不走 MCP | 发布 | 企业自建应用凭证 |
...
| `lx-nongfu` | 大文档按运营主体拆分到日常信息表，并按品牌+城市回填大文档 | 编排 | `lx_shujuku`, `lx-zhutichaibiao`, `lx-txsaasdocs`, `lx-tongzhi`, `lx-biaogetongbu` |
| `lx-hhdataimport` | hhdata 汇总数据导入（来源 A） | 数据导入 | `lxx_share` |
| `lx-lxdataimport` | lxdata 明细数据导入（来源 B），含邮箱拉取 | 数据导入 | `lxx_share` |
| `lx-dapanribao` | 运营日报生成（17 指标 + 异动检测），生成企业版发布计划 | 报表 | `lxx_share`, `lx_shujuku`, `lx-txsaasdocs` |
...
python .workbuddy/skills/lx-txdocs/scripts/publish_excel_folder.py \
  workspace/01主体拆表/输出/ --all-sheets
```
...
日报实际写入腾讯文档企业版时，后续使用 `lx-txsaasdocs` 读取 `dapanribao_publish_plan_{MMDD}.json` 后执行。目标根文件夹和表格命名规则写在 `config/fog_config.yaml` 的 `lx_dapanribao` 段；普通在线表格 range 写入端点仍需先验证。
...
- 个人版路径：拆表发布使用 `lx-txdocs` / `lxx_share.tdocs_api`
- 企业版路径：企业版 API 能力集中到 `lx-txsaasdocs`
