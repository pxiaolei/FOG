...
FOG 是 LXX 子模块项目，核心业务流程：**数据导入 → 拆表分发 → 报表生成 → 飞书云文档发布**。
...
| `lxx_share` | 共享 Python 库（DB连接/指标计算/Excel工具/缓存） | 基础设施 | `lx_shujuku`（码表） |
| `lx-init` | 旧初始化兼容入口；新入口是 `tools/fog.py` | 运维 | 无 |
| `lx-zhutichaibiao` | 按运营主体/城市/品牌拆表，生成各主体 Excel | 数据处理 | `lxx_share`, `lx_shujuku` |
| `lx-feishudocs` | 飞书云文档普通电子表格接入，通过 WorkBuddy 内置 lark-cli 写 Feishu Sheets | 发布 | WorkBuddy 飞书连接器 |
| `lx-biaogetongbu` | A 表到 B 表的登记/汇总同步，覆盖静默乘客、背审、拆表结果同步 | 数据处理 | `openpyxl` |
| `lx-tongzhi` | 面向商家/司机/线下渠道生成短信、push、微信/群通知和操作说明 | 触达内容 | 禁词 MD、上游业务事实 |
| `lx-nongfu` | 大文档按运营主体拆分到日常信息表，并按品牌+城市回填大文档 | 编排 | `lx_shujuku`, `lx-zhutichaibiao`, `lx-feishudocs`, `lx-tongzhi`, `lx-biaogetongbu` |
| `lx-hhdataimport` | hhdata 汇总数据导入（来源 A） | 数据导入 | `lxx_share` |
| `lx-lxdataimport` | lxdata 明细数据导入（来源 B），含邮箱拉取 | 数据导入 | `lxx_share` |
| `lx-dapanribao` | 运营日报生成（17 指标 + 异动检测），生成飞书普通表格发布计划 | 报表 | `lxx_share`, `lx_shujuku`, `lx-feishudocs` |
...
# 2. 如需线上协作，使用 lx-nongfu / lx-feishudocs 走飞书普通表格流程
```
...
日报实际写入飞书普通表格时，后续使用 `lx-feishudocs` 读取 `dapanribao_publish_plan_{MMDD}.json` 后执行。目标根文件夹和表格命名规则写在 `config/fog_config.yaml` 的 `lx_dapanribao` 段；本项目当前不使用飞书多维表格/Base/智能表格。
...
- 同事共享路径：飞书普通电子表格能力集中到 `lx-feishudocs`
- 不使用飞书多维表格/Base/智能表格；后续写表目标均为普通 Feishu Sheets
- 拆表发布目标结构：`飞书根文件夹/ → {运营主体}-运营主体/ → {运营主体}-日常信息`
- 日报发布目标结构：`飞书根文件夹/ → {运营主体}-运营主体/ → {运营主体}-大盘数据日报`
- 缓存文件（含 file_id/folder_id）不入版本控制：
  - `lx-feishudocs/assets/feishu_sheet_cache.json` — 飞书文件夹、spreadsheet token 和 sheet_id
  - `lx-dapanribao/assets/dailyreport_cache.json` — 日报表格兼容缓存
- 大表注意：写入普通飞书表格时按实际数据范围分块写入，不使用智能表格记录接口
...
| 拆表大文件发布失败？ | 先用 `lx-feishudocs` smoke 和 dry-run 验证普通表格权限、sheet 名和实际列数 |
| 飞书表格链接怎么识别？ | 使用飞书普通表格 URL 或 spreadsheet token，不使用 Base/智能表格链接 |
